...
def validation(sess, neuralnet, saver,
    ...
    for key in dataset.key_tot:
        ...
        print("* Validation of %s.dat" %(key))
        valcnt = 0
        start_time = time.time()
        while(True):

            D_tr, d_list = dataset.next_batch(batch_size=1, sequence_length=sequence_length, v_key=key)
            if(D_tr is None):
                break

            l2dist = sess.run(neuralnet.loss, feed_dict={neuralnet.inputs:D_tr, neuralnet.outputs:D_tr})
            list_dists.append(l2dist)
            list_bunchs.append(d_list)

            if(valcnt % 100 == 0):
                print("%s.dat [%d / %d] | L2 Dist: %f" %(key, valcnt, dataset.am_each, l2dist))
            valcnt += 1

        elapsed_time = time.time() - start_time
        print("Avg propagation time: %.3f" %(elapsed_time/valcnt))
